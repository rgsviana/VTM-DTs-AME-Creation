'''
	...
	EXEMPLO:
		python3 random_search-RAMIRO.py /home/rgsviana/DATASETS_AME/dataset_ame_16x16_train.csv /home/rgsviana/DATASETS_AME/dataset_ame_16x16_test.csv 3 dataset_ame_16x16
'''
import sys
import pandas as pd
...
import os
os.environ['JOBLIB_TEMP_FOLDER'] = '/home/rgsviana/DATASETS_AME/tmp/'
args = sys.argv
if len(args)!=5:
...
#X.drop(columns=['video_width', 'video_heigh', 'cu_width', 'cu_height', 'affine_pai', 'aff_viz_esq', 'aff_viz_esq_1', 'aff_viz_acima', 'aff_viz_acima_1', 'aff_inter_dir'], inplace=True)
#XT.drop(columns=['video_width', 'video_heigh', 'cu_width', 'cu_height', 'affine_pai', 'aff_viz_esq', 'aff_viz_esq_1', 'aff_viz_acima', 'aff_viz_acima_1', 'aff_inter_dir'], inplace=True)
X.drop(columns=['video_width', 'video_heigh'], inplace=True)
XT.drop(columns=['video_width', 'video_heigh'], inplace=True)
...
def classeToInt(row):
	if row['executaAffine'] == 0:
		return 0
	elif row['executaAffine'] == 1:
		return 1
	else:
		return 2
   
X['executaAffine'] = X.apply(lambda row: classeToInt(row),axis=1)
y = X['executaAffine']
X.drop(columns=['executaAffine'], inplace=True)
...
param_grid = {
	...
	'max_depth': [1] + list(range(5,101,1)),
